...
const canonicalUrl = "https://gyesanbox.kr/calculators/rent-vs-jeonse/";
...
export function buildRentVsJeonseResultText(
  input: RentVsJeonseInput,
  result: RentVsJeonseResult,
): string {
  return [
    "전세 vs 월세 비교 계산기",
    `전세보증금: ${formatWon(input.jeonseDeposit)}`,
    `전세대출금: ${formatWon(input.jeonseLoanAmount)} · 연 ${formatRate(input.jeonseLoanRate)}`,
    `월세: 보증금 ${formatWon(input.monthlyRentDeposit)} · 월 ${formatWon(input.monthlyRent)}`,
    `비교 기간: ${input.residenceMonths}개월`,
    `전세 월 환산 부담: ${formatWon(result.jeonseMonthlyBurden)}`,
    `월세 월 부담: ${formatWon(result.monthlyRentBurden)}`,
    `전세 총비용: ${formatWon(result.jeonseTotalCost)}`,
...
export function RentVsJeonseCalculator() {
  ...
  async function handleShare() {
    ...
    const text = buildRentVsJeonseResultText(calculatedInput, result);
    try {
      if (navigator.share) {
        await navigator.share({
          title: "전세 vs 월세 비교 계산기 결과",
          text,
          url: canonicalUrl,
        });
      ...
      {result && calculatedInput ? (
        <div className={styles.results}>
          ...
          <div className={styles.resultActions}>
            ...
            <button className={styles.calculateButton} onClick={handleCopy} type="button">
              결과 복사
            </button>
            <button className={styles.calculateButton} onClick={handleShare} type="button">
              결과 공유
            </button>
          </div>
