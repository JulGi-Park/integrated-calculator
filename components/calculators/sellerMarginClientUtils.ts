...
export function buildSellerMarginResultText(
  ...
  return [
    ...
    `할인금액: ${formatSellerMarginWon(input.sellerDiscount)}`,
    `고객 배송비: ${formatSellerMarginWon(input.customerShippingFee)}`,
    ...
    `판매자 배송비: ${formatSellerMarginWon(input.sellerShippingCost)}`,
    `광고비: ${formatSellerMarginWon(input.allocatedAdCost)}`,
    `기타 비용: ${formatSellerMarginWon(input.otherCost)}`,
    "",
    "[계산 결과]",
