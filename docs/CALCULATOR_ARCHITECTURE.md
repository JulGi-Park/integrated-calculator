...
한 계산기에서만 사용되는 구조는 기본적으로 전용으로 둔다. 두 계산기에서 실제 중복이 확인되고, 문구나 타입을 억지로 일반화하지 않아도 단순한 API를 만들 수 있을 때만 공통화한다. 현재 연봉·월급 실수령액 계산기 개발은 이 구조로 시작할 수 있으며, 두 번째 구현 후 입력 래퍼·결과 카드·브라우저 유틸리티의 실제 중복을 다시 평가한다.
