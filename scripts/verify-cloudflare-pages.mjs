...
const forbiddenPrivateOutputPaths = [
  ...
  "out/calculators/work-child-incentive.html",
];
...
async function verifyStaticOutput() {
  ...
  assert.doesNotMatch(
    sitemap,
    /roas|labor-pay|vat-profit|parental-leave|rent-vs-jeonse|car-cost|savings|average-price|brokerage-fee|card-installment|overtime-pay|work-child-incentive/,
  );
  ...
  for (const relativePath of publicHtmlFilesWithoutPrivateRoutes) {
    ...
    assert.doesNotMatch(
      html,
      /roas|labor-pay|vat-profit|parental-leave|rent-vs-jeonse|car-cost|savings|average-price|brokerage-fee|card-installment|overtime-pay|work-child-incentive|부동산 중개보수 계산기|근로·자녀장려금 계산기/,
    );
  }
}
