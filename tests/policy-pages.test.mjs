...
const pages = [
  {
    ...
    required: [
      "생활·금융·근로·사업·주거 계산의 기준과 결과를 함께 이해하도록 돕는 계산 전문 정보 서비스",
...
test("헤더에서 주요 탐색과 신뢰성 페이지로 이동할 수 있다", async () => {
  ...
  assert.match(source, /href: "\/contact\/"/);
  assert.match(source, /https:\/\/blog\.gyesanbox\.kr\//);
  assert.match(source, /aria-label="주요 메뉴"/);
  assert.match(source, /aria-controls="site-menu-panel"/);
  assert.match(source, /aria-label=\{open \? "메뉴 닫기" : "메뉴 열기"\}/);
});
...
test("변경 이력은 신규 10개 공개와 검증된 주요 변경을 올바르게 안내한다", async () => {
  ...
  const publicCalculators = [
    ...
    ["2026 4대보험 계산기", "/calculators/social-insurance/"],
    ["주휴수당 계산기", "/calculators/labor-pay/"],
    ["부가세 계산기", "/calculators/vat-profit/"],
    ["육아휴직급여 계산기", "/calculators/parental-leave/"],
  ...
  for (const [name, href] of publicCalculators) {
    ...
    await readFile(`app${href}page.tsx`, "utf8");
  }
  ...
  for (const { href } of sections) {
    const url = new URL(href, "https://gyesanbox.kr");
    assert.equal(url.protocol, "https:");
    assert.equal(url.host, "gyesanbox.kr");
  }
});
...
test("변경 이력은 신규 공개와 전세·월세 변경을 최신순으로 유지한다", () => {
  ...
  assert.match(html, /하한 410,000원·상한 6,590,000원/);
  assert.match(html, /13\.14%/);
  assert.match(html, /0\.9%/);
  assert.doesNotMatch(html, /13\.139999999999999|0\.8999999999999999/);
  assert.match(html, /주요 계산기 공개와 계산 기준·기능 변경 중/);
});
