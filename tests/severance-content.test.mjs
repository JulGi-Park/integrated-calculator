...
test("퇴직금 계산기 전용 SEO 메타데이터를 URL 하드코딩 없이 설정한다", () => {
...
test("관련 계산기는 실제 내부 라우트만 링크하고 준비 중 항목은 링크를 만들지 않는다", () => {
  ...
  assert.match(
    contentSource,
    /<article className=\{styles\.relatedCard\}>[\s\S]*시급·주휴·연장·야간수당 계산기/,
  );
  assert.doesNotMatch(
    contentSource,
    /href="(?:#|javascript:|\/calculators\/(?:unemployment|hourly|overtime|night))/,
  );
});
