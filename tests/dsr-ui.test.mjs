...
test("DSR 공식 산정 입력과 결과 항목을 렌더링한다", async () => {
  ...
  assert.ok(screen.getByLabelText("담보물 지역"));
  assert.ok(screen.getByLabelText("금리 유형"));
...
test("신용대출 일시상환 P1 fixture를 UI에서 계산한다", async () => {
  ...
  assert.ok(screen.getAllByText("24.8%").length >= 2);
  assert.ok(screen.getByText("20,000,000원"));
  assert.ok(screen.getAllByText("4,800,000원").length >= 2);
  assert.ok(screen.getAllByText("24,800,000원").length >= 1);
  assert.ok(screen.getByText("60개월"));
  assert.ok(screen.getByText("전체 신용대출 잔액"));
  assert.ok(screen.getByText("공식 스트레스 DSR"));
  assert.ok(screen.getByText(/1억원 이하이므로/));
});
...
test("결과 복사와 초기화가 공식 산정 결과를 유지한다", async () => {
  ...
  assert.match(copiedText, /공식 스트레스 DSR/);
