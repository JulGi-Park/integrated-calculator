...
import { isVehicleAcquisitionTaxCalculatorEnabled } from "../lib/calculators/vehicle-acquisition-tax/publication.ts";
...
test("정확한 소문자 true만 정적 route를 활성화한다", () => {
  ...
  assert.equal(isVehicleAcquisitionTaxCalculatorEnabled("true"), true);
  ...
  for (const value of [undefined, "", "false", "TRUE", "1", "yes"]) {
    ...
    assert.equal(isVehicleAcquisitionTaxCalculatorEnabled(value), false, String(value));
  }
});
  ...
  try {
    ...
    const removedRoutes = await pruneDisabledStaticRoutes({
      ...
      environment: {
        ...
        NEXT_PUBLIC_ENABLE_VEHICLE_ACQUISITION_TAX_CALCULATOR: "true",
      },
    });
