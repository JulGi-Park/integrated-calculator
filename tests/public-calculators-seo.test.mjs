...
test("공개 계산기 SEO 설정은 10개 페이지의 의도·이미지·키워드 후보를 고유하게 관리한다", async () => {
  ...
  for (const [slug, modulePath] of entries) {
    ...
    assert.doesNotMatch(pageSource, /CalculatorHeroImage/);
...
test("공개 계산기 컴팩트 히어로는 본문 이미지를 렌더링하지 않는다", async () => {
  const [component, css] = await Promise.all([
    readFile("components/common/CompactCalculatorHero.tsx", "utf8"),
    readFile("components/common/CompactCalculatorHero.module.css", "utf8"),
  ]);
