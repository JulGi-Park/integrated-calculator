...
const publicPageMetadataCases = [
  ...
  },
  {
    ...
    title: "계산기 모음 - 부가세·연봉·4대보험·대출 계산",
    description:
...
test("기존 공개 계산기 8개는 지정된 Open Graph와 Twitter 이미지를 사용한다", async () => {
...
test("육아휴직급여 계산기는 검증된 공용 OG 이미지를 사용한다", async () => {
  ...
  const image = "https://gyesanbox.kr/og/parental-leave-hero.png";
...
test("전세 vs 월세 계산기는 검증된 공용 OG 이미지를 사용한다", async () => {
  ...
  const image = "https://gyesanbox.kr/og/rent-vs-jeonse-hero.png";
