...
test("연봉 계산기 전용 SEO 메타데이터를 URL 하드코딩 없이 설정한다", () => {
  const expectedTitle =
...
test("판매자 계산기만 활성 링크이고 준비 중 계산기는 가짜 링크가 아니다", () => {
  assert.match(contentSource, /href="\/calculators\/seller-margin"/);
  assert.match(
    contentSource,
    /<article className=\{styles\.relatedCard\}>[\s\S]*퇴직금 계산기/,
  );
  assert.match(
    contentSource,
    /<article className=\{styles\.relatedCard\}>[\s\S]*실업급여 계산기/,
  );
  assert.doesNotMatch(
    contentSource,
    /href="(?:#|javascript:|\/calculators\/(?:retirement|severance|unemployment))/,
  );
});
