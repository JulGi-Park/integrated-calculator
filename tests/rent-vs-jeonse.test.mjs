...
test("라우트는 공개되며 홈·목록·sitemap·정적 검증에 등록된다", async () => {
  const [
    pageSource,
    listSource,
    homeSource,
...
test("과도한 금액·금리·기간과 소수 원 입력을 거부한다", () => {
  assertHasError({ ...baseInput, jeonseDeposit: 10_000_000_001 }, "jeonseDeposit", "OUT_OF_RANGE");
  assertHasError({ ...baseInput, monthlyRent: 1.5 }, "monthlyRent", "OUT_OF_RANGE");
  assertHasError({ ...baseInput, jeonseLoanRate: 100.1 }, "jeonseLoanRate", "OUT_OF_RANGE");
  assertHasError({ ...baseInput, residenceMonths: 601 }, "residenceMonths", "OUT_OF_RANGE");
});
