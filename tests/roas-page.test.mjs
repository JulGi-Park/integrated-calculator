...
test("ROAS는 sitemap, 메인 서비스 목록, 계산기 목록에 공개 연결되지 않는다", () => {
  const sitemapUrls = sitemapModule.default().map((entry) => entry.url);

  assert.doesNotMatch(sitemapSource, /\/calculators\/roas/);
  assert.equal(
    sitemapUrls.includes("https://gyesanbox.kr/calculators/roas"),
    false,
  );
  assert.doesNotMatch(calculatorListSource, /\/calculators\/roas|ROAS 계산기/);
  assert.doesNotMatch(homeSource, /\/calculators\/roas|ROAS 계산기/);
});
