...
test("Clipboard API로 고정 결과 텍스트를 복사하고 성공을 안내한다", async () => {
  ...
  assert.match(copiedText, /기준 확인일: 2026년 7월 11일/);
...
test("정책 연도·기준일·국민연금 현재 적용 기간을 표시한다", async () => {
  const user = userEvent.setup();
  renderCalculator();

  assert.ok(screen.getByText(/하한 410,000원/));
  assert.ok(screen.getByText(/상한 6,590,000원/));
  assert.ok(screen.getByText(/2026년 7월 1일/));
  assert.ok(screen.getByText(/2027년 6월 30일/));

  await enterStandardSalary(user);
  await user.click(screen.getByRole("button", { name: "실수령액 계산하기" }));
  const result = screen.getByRole("region", { name: "급여 실수령액" });
  assert.ok(within(result).getByText("2026년"));
  assert.ok(within(result).getByText("2026년 7월 11일"));
  assert.ok(
