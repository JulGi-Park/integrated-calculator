...
test("비공개 플래그는 정확히 문자열 true에서만 활성화된다", () => {
  const originalValue = process.env[DSR_POLICY.flagName];

  try {
    for (const value of ["TRUE", "yes", "1", "0", "false", ""]) {
      process.env[DSR_POLICY.flagName] = value;
      assert.equal(isDsrCalculatorEnabled(), false);
    }

    delete process.env[DSR_POLICY.flagName];
    assert.equal(isDsrCalculatorEnabled(), false);

    process.env[DSR_POLICY.flagName] = "true";
    assert.equal(isDsrCalculatorEnabled(), true);
  } finally {
    if (originalValue === undefined) {
      delete process.env[DSR_POLICY.flagName];
    } else {
      process.env[DSR_POLICY.flagName] = originalValue;
    }
  }
});
...
test("라우트는 비공개 플래그와 notFound 가드를 가진다", async () => {
  ...
  assert.match(source, /isDsrCalculatorEnabled/);
  assert.match(source, /notFound\(\)/);
  assert.match(source, /index:\s*false/);
});

test("sitemap, 목록, 공개 페이지에는 DSR 계산기를 노출하지 않는다", async () => {
  const files = [
    "app/sitemap.ts",
    "app/page.tsx",
  ...
  for (const source of sources) {
    assert.doesNotMatch(source, /\/calculators\/dsr|DSR 계산기 2026/);
  }
});
