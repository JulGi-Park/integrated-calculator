...
const routes = [
  ...
  [
    "app/calculators/parental-leave/page.tsx",
    "육아휴직급여 계산기 | 월 통상임금 기준 예상 급여 계산",
  ],
  [
    "app/calculators/rent-vs-jeonse/page.tsx",
...
test("부가세 계산기 페이지와 목록 링크가 실제 UI를 제공한다", async () => {
  ...
  assert.match(pageSource, /<CompactCalculatorHero/);
  assert.match(pageSource, /title="부가세 계산기"/);
  assert.match(pageSource, /JsonLdScripts/);
  assert.doesNotMatch(pageSource, /notFound\(\)|계산 기능 준비 중/);
...
test("실업급여 계산기 페이지와 목록 링크가 실제 UI를 제공한다", async () => {
  ...
  assert.match(pageSource, /<CompactCalculatorHero/);
  assert.match(pageSource, /title="실업급여 계산기"/);
  assert.match(pageSource, /JsonLdScripts/);
  assert.doesNotMatch(pageSource, /계산 기능 준비 중/);
...
test("퇴직금 계산기 페이지와 목록 링크가 실제 UI를 제공한다", async () => {
  ...
  assert.match(pageSource, /<CompactCalculatorHero/);
  assert.match(pageSource, /title="퇴직금 계산기"/);
  assert.match(pageSource, /JsonLdScripts/);
  assert.doesNotMatch(pageSource, /계산 기능 준비 중/);
...
test("육아휴직급여 계산기 페이지와 목록 링크가 실제 UI를 제공한다", async () => {
  const [pageSource, listSource] = await Promise.all([
    readFile("app/calculators/parental-leave/page.tsx", "utf8"),
    readFile("app/calculators/page.tsx", "utf8"),
  ]);

  assert.match(pageSource, /ParentalLeaveCalculator/);
  assert.match(pageSource, /<CompactCalculatorHero/);
  assert.match(pageSource, /title="육아휴직급여 계산기"/);
  assert.match(pageSource, /JsonLdScripts/);
  assert.doesNotMatch(pageSource, /notFound\(\)|계산 기능 준비 중/);
  assert.match(listSource, /href="\/calculators\/parental-leave\/"/);
});

test("전세 vs 월세 계산기 페이지와 목록 링크가 실제 UI를 제공한다", async () => {
  const [pageSource, listSource] = await Promise.all([
    readFile("app/calculators/rent-vs-jeonse/page.tsx", "utf8"),
    readFile("app/calculators/page.tsx", "utf8"),
  ]);
  assert.match(pageSource, /RentVsJeonseCalculator/);
  assert.match(pageSource, /<CompactCalculatorHero/);
  assert.match(pageSource, /title="전세 vs 월세 비교 계산기"/);
  assert.doesNotMatch(pageSource, /notFound\(\)|계산 기능 준비 중/);
  assert.match(listSource, /href="\/calculators\/rent-vs-jeonse\/"/);
});
