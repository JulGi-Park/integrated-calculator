...
import { metadata } from "../app/calculators/work-child-incentive/page.tsx";
import {
...
test("근로·자녀장려금 계산기는 전용 OG/Twitter 이미지를 사용한다", async () => {
  const image = "https://gyesanbox.kr/og/work-child-incentive.png";
  const png = await readFile("public/og/work-child-incentive.png");
  assert.equal(metadata.openGraph.url, "https://gyesanbox.kr/calculators/work-child-incentive/");
  assert.deepEqual(metadata.openGraph.images, [{ url: image, width: 1200, height: 630, alt: "근로·자녀장려금 계산기" }]);
