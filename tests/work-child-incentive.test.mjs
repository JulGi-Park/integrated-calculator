...
import {
  ...
  isWorkChildIncentiveCalculatorEnabled,
  WORK_CHILD_INCENTIVE_POLICY,
...
test("비공개 플래그는 정확히 문자열 true에서만 활성화된다", () => {
  ...
  try {
    ...
    delete process.env[WORK_CHILD_INCENTIVE_POLICY.flagName];
    assert.equal(isWorkChildIncentiveCalculatorEnabled(), false);

    process.env[WORK_CHILD_INCENTIVE_POLICY.flagName] = "true";
    assert.equal(isWorkChildIncentiveCalculatorEnabled(), true);
  } finally {
    if (originalValue === undefined) {
      delete process.env[WORK_CHILD_INCENTIVE_POLICY.flagName];
    } else {
      process.env[WORK_CHILD_INCENTIVE_POLICY.flagName] = originalValue;
    }
  }
});
...
test("라우트는 비공개 플래그와 notFound 가드를 가진다", async () => {
  ...
  assert.match(source, /isWorkChildIncentiveCalculatorEnabled/);
...
test("sitemap, 목록, 공개 페이지에는 신규 계산기를 노출하지 않는다", async () => {
  const files = [
    "app/sitemap.ts",
    "app/page.tsx",
  ...
  for (const source of sources) {
    assert.doesNotMatch(source, /\/calculators\/work-child-incentive|근로·자녀장려금 계산기/);
  }
});

test("Cloudflare 검증은 신규 비공개 산출물을 pruning 대상으로 차단한다", async () => {
  ...
  assert.match(source, /prunePrivateStaticOutput/);
});
...
test("공식 출처와 기준일, 면책 문구를 콘텐츠에 포함한다", async () => {
  ...
  assert.match(constantsSource, /2026-07-08/);
