...
test("6+6 특례 적용 가능 결과는 월별 특례와 일반 기준 적용 구간을 표시한다", async () => {
  const user = userEvent.setup();
  renderCalculator();
  ...
  assert.ok(screen.getByText("일반 기준 적용 구간"));
...
test("6+6 특례 적용 불가 결과는 사유와 일반 기준 적용 안내를 표시한다", async () => {
  const user = userEvent.setup();
  renderCalculator();
...
test("한부모 특례 적용 불가 결과는 사유와 일반 기준 적용 안내를 표시한다", async () => {
  const user = userEvent.setup();
  renderCalculator();
...
test("공유 취소 AbortError는 실패가 아니라 취소 안내로 처리한다", async () => {
