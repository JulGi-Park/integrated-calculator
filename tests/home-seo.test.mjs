...
test("홈 JSON-LD는 화면 내용과 연락처 및 구현 완료 계산기만 담는다", () => {
  ...
  assert.equal(scripts[1].alternateName, undefined);
  ...
  assert.equal(new Set(scripts[1].sameAs).size, scripts[1].sameAs.length);
  assert.ok(scripts[1].sameAs.every((url) => /^https:\/\/www\.(instagram\.com\/gyesanbox\/|threads\.com\/@gyesanbox)$/.test(url)));
  assert.equal(scripts[1].contactPoint["@type"], "ContactPoint");
  assert.equal(scripts[1].contactPoint.url, "https://gyesanbox.kr/contact/");
  assert.equal(scripts[1].contactPoint.contactType, "customer support");
  assert.equal(scripts[1].contactPoint.availableLanguage, "ko");
  assert.deepEqual(
  ...
  assert.match(html, /계산박스는 gyesanbox\.kr에서 운영하는 생활·금융·근로 계산 서비스입니다/);
  assert.doesNotMatch(html, /계산박스\(gyesanbox\)/);
});
