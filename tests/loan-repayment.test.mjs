...
function assertScheduleInvariants(result, principal, termMonths) {
  ...
  for (let index = 0; index < result.schedule.length; index += 1) {
    ...
    assert.ok(item.monthlyPayment >= 0);
  ...
  assert.ok(Number.isSafeInteger(result.totalInterest));
  assert.ok(Number.isSafeInteger(result.totalPayment));
}
...
test("고정 사례 1의 원리금균등 첫 회차와 마지막 회차를 검증한다", () => {
  ...
  assert.deepEqual(result.schedule[179], {
    installmentNumber: 180,
    openingBalance: 66_491_376,
    principalPayment: 257_342,
    interestPayment: 249_343,
    monthlyPayment: 506_685,
    closingBalance: 66_234_034,
  });
});
...
test("고정 사례 1의 원금균등 첫 회차와 마지막 회차를 검증한다", () => {
  ...
  assert.deepEqual(result.schedule[179], {
    installmentNumber: 180,
    openingBalance: 50_277_917,
    principalPayment: 277_777,
    interestPayment: 188_542,
    monthlyPayment: 466_319,
    closingBalance: 50_000_140,
  });
});
...
test("공식 고정 사례 3: 나누어떨어지지 않는 원금과 소수점 4자리 금리를 검증한다", () => {
  const data = assertSuccess(
    calculateLoanRepaymentComparison({
      principal: 1_000_001,
      annualInterestRate: 3.7525,
      termMonths: 7,
    }),
  );
  ...
  assert.deepEqual(
    ...
    {
      equalPayment: [12_548, 144_650, 144_649],
      equalPrincipal: [12_508, 145_984, 143_306],
      bullet: [21_889, 3_127, 1_003_128],
    },
  );
});
...
test("소수점 이하 4자리 연이율을 정상 허용한다", () => {
  const response = calculateLoanRepaymentComparison({
    ...baseInput,
    annualInterestRate: 4.1234,
  });
...
test("0%·1개월 대출은 세 방식 모두 이자 없이 원금만 상환한다", () => {
  ...
  for (const result of [
    ...
    assertScheduleInvariants(result, 1_000_001, 1);
    assert.equal(result.totalInterest, 0);
    assert.equal(result.totalPayment, 1_000_001);
    assert.equal(result.schedule[0].interestPayment, 0);
  }
});
...
test("회차 이자가 0.5원 미만이면 원 단위로 내림한다", () => {
  const data = compareLoanRepayments({
    principal: 5,
    annualInterestRate: 100,
    termMonths: 1,
  ...
  assert.equal(data.equalPayment.schedule[0].interestPayment, 0);
  assert.equal(data.equalPrincipal.schedule[0].interestPayment, 0);
  assert.equal(data.bullet.schedule[0].interestPayment, 0);
});
...
test("1개월 대출의 첫 달 부담 동률을 결정적 방식 순서로 모두 반환한다", () => {
  const input = {
    principal: 1_000_000,
    annualInterestRate: 4.5,
    termMonths: 1,
  };
  const expectedOrder = ["equalPayment", "equalPrincipal", "bullet"];

  for (let run = 0; run < 3; run += 1) {
    const data = compareLoanRepayments(input);
    assert.deepEqual(data.lowestFirstMonthPaymentTypes, expectedOrder);
  }
});
...
test("최대 입력의 600개월 일정은 반복 실행과 직렬화에 무리가 없다", (t) => {
  const input = {
    principal: LOAN_REPAYMENT_POLICY.maximumPrincipal,
    annualInterestRate: LOAN_REPAYMENT_POLICY.maximumAnnualInterestRate,
    termMonths: LOAN_REPAYMENT_POLICY.maximumTermMonths,
  ...
  for (let run = 0; run < 3; run += 1) {
    const data = compareLoanRepayments(input);
    assert.equal(data.equalPayment.schedule.length, 600);
    assert.equal(data.equalPrincipal.schedule.length, 600);
    assert.equal(data.bullet.schedule.length, 600);
    const serialized = JSON.stringify(data);
  ...
  assert.ok(Number.isFinite(elapsedMilliseconds));
  assert.ok(serializedLength < 1_000_000);
  t.diagnostic(
    `최대 입력 3회 계산·직렬화: ${elapsedMilliseconds.toFixed(2)}ms, JSON ${serializedLength.toLocaleString("en-US")} bytes`,
  );
});
